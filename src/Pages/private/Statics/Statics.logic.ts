...
export const useStaticsLogic = (students: Student[], staticsData?: any) => {
  ...
  try {
    if (staticsData && typeof staticsData === 'object') {
      overriddenTotalStudents = staticsData.numero_de_estudiantes ?? null;

      const est = staticsData.estudiante_con_mayor_cantidad_de_faltas ?? null;
      if (est) {
      ...
      const cursoName = staticsData.curso_con_mayor_cantidad_de_faltas ?? null;
      const cursoCount =
        staticsData.curso_con_mayor_cantidad_de_faltas_count ?? 0;
      if (cursoName)
